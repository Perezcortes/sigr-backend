...
import { LoginDto, RegisterDto, RefreshTokenDto, ChangePasswordDto, LoginResponseDto, RefreshResponseDto, ForgotPasswordDto, ResetPasswordDto } from "./dto/auth.dto";
...
    
    const genericMessage = "Si tu correo está en nuestro sistema, te hemos enviado un enlace para restablecer tu contraseña. Revisa tu bandeja de entrada y spam.";
    
...
    // Generar un token seguro (UUID) y su hash para guardar en la base de datos
...
    
    // Busca todos los tokens no usados.
    const allUnusedTokens = await this.passwordResetTokenRepository.find({
      where: { is_used: false },
      relations: ["user"],
    });
    
    let foundTokenRecord: PasswordResetToken | null = null;
    
    // Itera sobre los tokens para encontrar una coincidencia con el hash
...
    await this.userRepository.save(user);
    
    // Invalidar el token después de un uso exitoso
